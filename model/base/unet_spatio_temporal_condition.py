...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        # 按照SDV ControlNet的模式：在所有down blocks完成后统一处理ControlNet残差
        if down_block_additional_residuals is not None:
            ...
            for down_block_res_sample, down_block_additional_residual in zip(
                ...
                down_block_res_sample = down_block_res_sample + down_block_additional_residual.to(down_block_res_sample.dtype)
        ...
        # 处理ControlNet mid block残差
        if mid_block_additional_residual is not None:
            sample = sample + mid_block_additional_residual.to(sample.dtype)
