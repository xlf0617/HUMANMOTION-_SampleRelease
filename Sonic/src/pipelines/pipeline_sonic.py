...
class SonicPipeline(DiffusionPipeline):
    ...
    def _clip_encode_image(self, image, audio_prompts, uncond_audio_prompts, num_frames, device, num_videos_per_prompt,
                           do_classifier_free_guidance, frames_per_batch):
    ...
    def _encode_video(self, video: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            latents = self.vae.encode(video).latent_dist.meam()
    ...
    def __call__(
        ...
        latents = self._encode_video(latents)
            ...
            for i, t in enumerate(timesteps):
                ...
                for batch, index_start in enumerate(range(0, num_frames, frames_per_batch - overlap)):
                    ...
                    noise_pred = self.unet(
                        ...
                        batch_image_embeddings.flatten(0, 1), [batch_audio_prompts.flatten(0, 1)]),
                    ...
                    if do_classifier_free_guidance:
                        ...
                        noise_pred = noise_pred_uncond + self.guidance_scale1[i] * (
                                    noise_pred_drop_audio - noise_pred_uncond) + self.guidance_scale2[i] * (
